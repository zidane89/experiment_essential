...
class Environment:
    ...
    def post_process(self, action, p_stack, p_bat, p_mot, m_fuel):
        done = self.update_soc(p_bat, p_stack)
        if done:
            reward = -1000
        else:
    ...
    def get_curdensity_region(self, p_mot):
        ...
        P_battery_set = np.min(p_mot, 0) - P_stack_set
    ...
    def update_soc(self, p_bat, p_stack):
        p_stack = - p_stack
        done = False
        ...
        if not condition:
            ...
            del_i_stack = (1 / (2 * r_cha)) * (v_cha - (v_cha ** 2 - 4 * r_cha * p_stack) ** (0.5)) * (p_stack < 0) + \
                          (1 / (2 * r_dis)) * (v_dis - (v_dis ** 2 - 4 * r_dis * p_bat) ** (0.5)) * (p_stack >= 0)
            del_soc = -(del_i + del_i_stack) * (self.calculation_comp["del_t"] / self.battery['Q_cap'])
